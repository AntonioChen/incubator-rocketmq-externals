...
module.controller('message4EventController', ['$scope', 'ngDialog', '$http','Notification', '$routeParams',function ($scope, ngDialog, $http,Notification, $routeParams) {
    $scope.allTopicList = [];
    $scope.selectedTopic =[];
    $scope.key ="";
    $scope.messageId ="";
    ...
    $scope.queryMessageByBrokerAndOffset = function (storeHost,commitLogOffset) {
        $http({
            ...
            if (resp.status == 0) {
                ...
                ngDialog.open({
                    template: 'message4EventDetailViewDialog',
                    controller: 'message4EventDetailViewDialogController',
                    data: resp.data
                });
            } else {
                Notification.error({message: resp.errMsg, delay: 2000});
...
module.controller('message4EventDetailViewDialogController',['$scope', 'ngDialog', '$http','Notification', function ($scope, ngDialog, $http,Notification) {
